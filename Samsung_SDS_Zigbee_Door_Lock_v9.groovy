/**
 *  Samsung SDS Zigbee Door Lock Driver for Hubitat (v9)
 *  Supports: SHP-960 Plus, SHP-DP960SG (DADT302 Zigbee module)
 *  Manufacturer: SAMSUNG SDS
 *
 *  v9 changes:
 *   - Battery percentage is now hardcoded to 100 instead of being computed
 *     from voltage. Confirmed by the original edge-driver author (스피드박)
 *     in community comments: even the WiFi-module (Zigbang app) path can't
 *     get a reliable battery reading either - every lock/unlock draws a
 *     large motor current that causes a brief voltage dip/recovery, so
 *     whatever voltage happens to be sampled swings too much to show even a
 *     believable downward trend, let alone an accurate percentage. This
 *     matches Samsung's own official DTH, which also just hardcodes
 *     battery=100 in configure() rather than computing it. So: showing a
 *     wrong/noisy number is worse than showing a fixed 100 and relying on
 *     the lock's own physical low-battery voice/LED alert (which is the
 *     one thing everyone agrees is trustworthy). batteryVoltage is still
 *     reported for anyone curious, just no longer used to derive battery %.
 *
...
metadata {
    definition(name: "Samsung SDS Zigbee Door Lock v9", namespace: "community", author: "community") {
        capability "Lock"
...
def initialize() {
    ...
    // v9: default to 100% right away rather than waiting on a voltage read
...
// v9: battery % is hardcoded to 100 (see header notes) - voltage swings too
// much around each motor actuation to derive a trustworthy percentage.
// batteryVoltage is still reported for reference/curiosity only.
def parseBatteryVoltage(String value) {
    if (!value) return null
    def raw = Integer.parseInt(value, 16)
    def volts = raw / 10.0
    def events = []
    events << createEvent(name: "batteryVoltage", value: volts, unit: "V", descriptionText: "${device.displayName} battery voltage is ${volts}V (reference only, not used for %)")
    events << createEvent(name: "battery", value: 100, unit: "%", descriptionText: "${device.displayName} battery fixed at 100% - rely on the lock's own low-battery alert instead")
    return events
}
